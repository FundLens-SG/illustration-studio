/* =========================================================================
   Illustration Studio · Paper Deck Builder
   ------------------------------------------------------------------------
   Shared module used by BOTH the in-browser Print button and the
   Playwright build script (`scripts/build-paper-pdf.mjs`). Builds a
   6-page A4 landscape paper deck from the live calculated state.

   Exposes:
     window.IllustrationStudioPaperDeck = {
       extractDomData(),           // pull visual values + chart PNG from the live DOM
       buildHtml(data, snapshot),  // assemble full HTML document for the deck
       print(),                    // open a fresh window and trigger window.print()
     }

   Pages:
     1 · Executive Summary
     2 · Value Mechanics  (Welcome Bonus / Charges / 101% Protection)
     3 · Projection Chart
     4 · Annual Projection Table  (milestones only, multi-page if --full-table)
     5 · Withdrawal & Access Summary
     6 · Important Disclaimers  (own page, break-before: page)

   The deck CSS is self-contained — no external fonts, hairline panels on
   tinted paper, no UI controls leak through.
   ========================================================================= */

(function (root) {
  "use strict";

  // Default A4 landscape page math:
  //   210mm height − 10mm top pad − 12mm bottom pad − 14mm head
  //   − 6mm head margin − 14mm foot reserve = ~154mm content
  ...
  function currencySymbol(currency) { return currency === "USD" ? "US$" : "S$"; }
  ...
  function pageShell({ pageNo, totalPages, title, kicker = "Private Client Illustration", body, extraClass = "", scenarioName = "", logoSrc = "", generatedAt = "" }) {
    const lockupKicker = scenarioName ? `${scenarioName} · ${kicker}` : kicker;
    return `
      <section class="paper-page ${extraClass}">
        <header class="paper-head">
          <div class="paper-lockup">
            <img class="paper-logo" src="${esc(logoSrc)}" alt="" />
            <div>
  ...
  function executiveSummaryPage({ data, snapshot, pageNo, totalPages, common }) {
  ...
  function valueMechanicsPage({ snapshot, pageNo, totalPages, common }) {
      ...
      body: `
        <main class="value-mechanics-grid">
          <section class="value-card welcome-card">
            ...
            <div class="value-card-big">${esc(fmtMoney(bonuses.welcomeBonusAmount || 0, currency))}</div>
            <div class="value-card-sub">Year 1 bonus credited to the policy account</div>
            <div class="value-card-lines">
              ${lineRow("Welcome bonus rate", fmtPercent(bonuses.welcomeRate || 0, 1), `On ${fmtMoney(snapshot?.settings?.annualizedPremium || 0, currency)} annualised premium`)}
          ...
          <section class="value-card charges-card">
            ...
            <div class="value-card-big">${esc(fmtMoney(charges.totalModeledCharges || 0, currency))}</div>
          ...
          <section class="value-card protection-card">
            ...
            <div class="value-card-lines">
              ${lineRow("Death/TI benefit at end-age", fmtMoney(protection.deathBenefit || 0, currency))}
  ...
  function chartPage({ data, snapshot, pageNo, totalPages, common }) {
  ...
  function annualTablePage({ data, rows, chunkIndex, chunkCount, pageNo, totalPages, common }) {
    const headerHtml = `
      <thead>
        <tr>
      ...
      body: `
        <main class="access-page">
          <section class="panel access-panel-print">
            ...
            <div class="access-grid">${accessCardsHtml}</div>
          </section>
            ...
            <div class="rules-two-col">
              ...
              <div>
                ...
                <ul class="access-notes">
                  ...
                  <li>Surrender values shown in this deck are net of admin and surrender charge.</li>
  ...
  function disclaimersPage({ data, pageNo, totalPages, common }) {
    return pageShell({
      ...common, pageNo, totalPages,
  ...
  const DECK_CSS = `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    html, body {
      margin: 0; padding: 0;
      background: #ffffff; color: #101a28;
    ...
    .paper-head {
      ...
      height: 14mm; border-bottom: 1px solid #dedbd4; margin-bottom: 6mm;
    }
    .paper-lockup { display: flex; align-items: center; gap: 9px; min-width: 0; }
    .paper-logo { width: 26px; height: 26px; border-radius: 6px; }
    .paper-kicker, .section-kicker, .metric-label, .paper-foot, .paper-title b, .panel-head span {
      font-family: "SFMono-Regular", "Roboto Mono", "Geist Mono", Consolas, monospace;
      font-size: 7.4pt; letter-spacing: 0.12em;
      text-transform: uppercase; color: #7a7f87;
    }
    .paper-head h1 { margin: 1px 0 0; font-size: 12.5pt; line-height: 1; font-weight: 600; color: #1a1d22; }
    .paper-title { display: flex; align-items: center; gap: 12px; color: #454a51; font-size: 9.5pt; font-weight: 600; }
    .paper-title b { font-weight: 500; color: #8e6936; }
    .paper-foot {
      position: absolute; left: 12mm; right: 12mm; bottom: 6mm;
      display: flex; justify-content: space-between;
      border-top: 1px solid #e8e5dc; padding-top: 3mm;
      font-size: 6.8pt; color: #8a8d93;
    }

    .panel, .metric-card, .hero-panel, .value-card {
      border: 1px solid #e5e5e3; border-radius: 8px; background: #ffffff;
    }
    .panel-head { display: flex; align-items: baseline; justify-content: space-between; gap: 10px; margin-bottom: 4mm; }
    .panel-head h2 { margin: 0; color: #1a1d22; font-size: 11.5pt; font-weight: 600; }
    .panel { padding: 6mm; }
    .wide { grid-column: 1 / -1; }

    .exec-grid {
      display: grid; grid-template-columns: 1.1fr 1fr;
      grid-auto-rows: min-content; gap: 5mm;
      height: calc(210mm - 10mm - 12mm - 14mm - 6mm - 14mm);
    }
    .hero-panel { padding: 8mm; grid-row: span 2; display: flex; flex-direction: column; }
    .hero-value {
      margin-top: 4mm; font-size: 34pt; line-height: 1; font-weight: 500;
      letter-spacing: -0.035em; color: #101a28; font-variant-numeric: tabular-nums lining-nums;
    }
    .hero-caption { margin: 5mm 0 0; color: #454a51; font-size: 10.5pt; }
    .hero-foot { margin-top: auto; display: grid; grid-template-columns: repeat(3, 1fr); gap: 4mm; padding-top: 5mm; border-top: 1px solid #eeeeec; }
    .metric-value-sm {
      font-family: "SFMono-Regular", "Roboto Mono", "Geist Mono", Consolas, monospace;
      font-size: 13pt; font-weight: 600; color: #1a1d22;
      font-variant-numeric: tabular-nums lining-nums; margin-top: 2mm;
    }
    .metric-value-sm.brass { color: #8e6936; }

    .exec-meta { display: grid; grid-template-columns: repeat(2, 1fr); gap: 4mm; }
    .metric-card { min-height: 22mm; padding: 4mm 5mm; }
    .metric-value {
      margin-top: 2mm;
      font-family: "SFMono-Regular", "Roboto Mono", "Geist Mono", Consolas, monospace;
      font-size: 13pt; font-weight: 600; color: #1a1d22;
    ...
    .print-comp-bar { display: flex; height: 8mm; overflow: hidden; border-radius: 4px; background: #f3f3f1; }
    .print-comp-bar span {
      display: flex; align-items: center; justify-content: center;
      min-width: 3px; color: #ffffff; font-size: 7pt; font-weight: 600; white-space: nowrap;
    }
    .composition-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 1mm 8mm; margin-top: 3mm; }
    .composition-row { display: grid; grid-template-columns: 10px 1fr auto; align-items: center; gap: 8px; padding: 1.8mm 0; border-bottom: 1px solid #eeeeec; }
    .composition-row i { width: 10px; height: 10px; border-radius: 2px; }
    .composition-row span { color: #454a51; font-size: 9pt; }
    .composition-row b { font-family: "SFMono-Regular", "Roboto Mono", "Geist Mono", Consolas, monospace; font-size: 9pt; font-weight: 600; color: #1a1d22; }

    .value-mechanics-grid {
      display: grid; grid-template-columns: repeat(3, 1fr); gap: 5mm;
      height: calc(210mm - 10mm - 12mm - 14mm - 6mm - 14mm);
    }
    .value-card { padding: 8mm; display: flex; flex-direction: column; }
    .value-card-big {
      margin: 3mm 0 1mm; font-size: 28pt; line-height: 1; font-weight: 500;
      letter-spacing: -0.025em; color: #1a1d22; font-variant-numeric: tabular-nums lining-nums;
    }
    .value-card-big.brass { color: #8e6936; }
    .value-card-sub { color: #5f6670; font-size: 9.4pt; margin-bottom: 5mm; }
    .value-card-lines { display: flex; flex-direction: column; flex: 1; }
    .line-row { display: flex; justify-content: space-between; gap: 6mm; padding: 2.4mm 0; border-top: 1px solid #eeeeec; font-size: 9pt; }
    .line-row span { color: #454a51; }
    .line-row span small { display: block; color: #8a8d93; font-size: 7.6pt; margin-top: 0.5mm; letter-spacing: 0; }
    .line-row b { font-family: "SFMono-Regular", "Roboto Mono", "Geist Mono", Consolas, monospace; color: #1a1d22; font-weight: 600; white-space: nowrap; align-self: start; }
    .value-card-note, .value-card-warn { margin: 4mm 0 0; font-size: 8.2pt; color: #5f6670; line-height: 1.45; }
    .value-card-warn { background: #fff7e8; border: 1px solid #f0d9a3; padding: 3mm; border-radius: 4px; color: #6b4f1c; }
    .welcome-card { background: #fff; }
    .charges-card { background: #fbfaf3; }
    .protection-card { background: #0f1d2d; color: #f5edd4; border-color: #1f2c44; }
    ...
    .protection-card .value-card-warn { background: rgba(216, 200, 149, 0.10); border-color: rgba(216, 200, 149, 0.35); color: #f4ecd5; }

    .chart-page { display: grid; grid-template-columns: 1fr; gap: 5mm; height: calc(210mm - 10mm - 12mm - 14mm - 6mm - 14mm); }
    .chart-panel-print { padding: 6mm 7mm; }
    .chart-legend-print { display: flex; gap: 5mm; color: #69727d; font-size: 8pt; margin-bottom: 2mm; }
    ...
    .disclaimer-list { margin: 0; padding-left: 6mm; columns: 2; column-gap: 10mm; }
    .disclaimer-list li { break-inside: avoid; margin: 0 0 3mm; color: #454a51; font-size: 8.4pt; line-height: 1.45; }
    .disclaimer-list strong { display: block; color: #1a1d22; margin-bottom: 0.6mm; }
  `;
  ...
  function buildHtml(data, snapshot) {
    const common = {
      ...
      scenarioName: (snapshot && snapshot.scenarioName) || data.scenarioName || "",
    };
    ...
    const pages = [];
    let pageNo = 1;
    pages.push(executiveSummaryPage({ data, snapshot, pageNo: pageNo++, totalPages, common }));
    pages.push(valueMechanicsPage({ snapshot, pageNo: pageNo++, totalPages, common }));
    pages.push(chartPage({ data, snapshot, pageNo: pageNo++, totalPages, common }));
    annualChunks.forEach((rows, idx) => {
      pages.push(annualTablePage({ data, rows, chunkIndex: idx, chunkCount: annualChunks.length, pageNo: pageNo++, totalPages, common }));
    });
    pages.push(withdrawalAccessPage({ data, snapshot, pageNo: pageNo++, totalPages, common }));
    pages.push(disclaimersPage({ data, pageNo: pageNo++, totalPages, common }));
...
<body class="paper-deck">
  ${pages.join("\n")}
  <script>
    // Auto-print as soon as the chart image has loaded.
    (function () {
      var imgs = document.images;
      var pending = 0;
      function done() {
        if (--pending <= 0) {
          // give the layout one frame to settle, then print
          requestAnimationFrame(function () { setTimeout(function () { window.focus(); window.print(); }, 80); });
        }
      }
  ...
  function extractDomData() {
    ...
    return {
      ...
      composition, segments, chartPng,
      chartEndLabel: idText("chartEndLabel"),
      chartLegend, annualMode, annualHeaders, annualRows, accessCards, disclaimers,
    };
  }

  // Open a fresh print window with the deck HTML and trigger window.print()
  // once the chart image has loaded. The user picks "Save as PDF" in the
  // print dialog (Layout: Landscape is enforced via @page).
  function openPrintWindow() {
    const snapshot = (root.IllustrationStudioPrint && root.IllustrationStudioPrint.getSnapshot)
      ? root.IllustrationStudioPrint.getSnapshot()
      : null;
    if (!snapshot) {
      window.alert("The illustration hasn't computed yet — try again in a moment.");
      return;
    }
    const data = extractDomData();
    const html = buildHtml(data, snapshot);
    const win = window.open("", "_blank", "width=1280,height=900");
    if (!win) {
      window.alert("Pop-ups appear to be blocked. Allow pop-ups for this site, then click Print PDF again.");
      return;
    }
    win.document.open();
  ...
  root.IllustrationStudioPaperDeck = {
    buildHtml,
    extractDomData,
